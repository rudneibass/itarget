...
import { DomainException } from '@src/account/domain/exceptions/domain.exception';
...
export class AddUserService {
  ...
  constructor(
    ...
    @Inject('HashAdapterInterface') private readonly hashProviderAdapter: HashAdapterInterface,
  ...
  private async persist(createUserDto: CreateUserServiceInputDto){
    const passwordHash = await this.hashProviderAdapter.hash(createUserDto.password)
  ...
  private async validadeUniqueEmail(email: string){
