import { getFormCreate } from './index';
...
describe('getFormCreate', () => {
  ...
  it('should successfully get form with formName', async () => {
    ...
    const result = await getFormCreate({ endpoint: mockEndpoint, formName: mockFormName });
  ...
  it('should successfully get form without formName', async () => {
    ...
    const result = await getFormCreate({ endpoint: mockEndpoint });
  ...
  it('should handle Axios error with response data', async () => {
    ...
    await expect(getFormCreate({ endpoint: mockEndpoint, formName: mockFormName }))
  ...
  it('should handle empty endpoint', async () => {
    ...
    const result = await getFormCreate({ endpoint: emptyEndpoint, formName: mockFormName });
  ...
  it('should handle empty formName', async () => {
    ...
    const result = await getFormCreate({ endpoint: mockEndpoint, formName: emptyFormName });

    expect(api.get).toHaveBeenCalledWith(mockEndpoint, { params: { form_name: emptyFormName } });
    expect(result).toEqual(mockResponse.data);
  });

  it('should handle null formName', async () => {
    (api.get as jest.Mock).mockResolvedValueOnce(mockResponse);

    const result = await getFormCreate({ endpoint: mockEndpoint, formName: null as any });

    expect(api.get).toHaveBeenCalledWith(mockEndpoint, { params: { form_name: '' } });
    expect(result).toEqual(mockResponse.data);
  });

  it('should handle undefined formName', async () => {
    (api.get as jest.Mock).mockResolvedValueOnce(mockResponse);

    const result = await getFormCreate({ endpoint: mockEndpoint, formName: undefined });
